...
txargs0 = {"from": acc0, "gasPrice": web3.toWei('5', 'gwei'), "gas": 8000000, "nonce": nonce0}
#txargs0 = {"from": acc0, "gasPrice": 1, "gas": 8000000, "nonce": nonce0}

# este lo uso para mandar desde el user
txargs1 = {"from": acc1, "gasPrice": web3.toWei('5', 'gwei'), "gas": 10000000, "nonce": nonce1}
#txargs1 = {"from": acc1, "gasPrice": 2, "gas": 6000000, "nonce": nonce1}

with open("../../Contratos/Tests/Compilado/trash.abi") as contract_abi_file:
  contract_abi = json.load(contract_abi_file)
with open("../../Contratos/Tests/Compilado/trash.bin") as contract_bin_file:
...
rv_txn = contract_instance.constructor().buildTransaction(transaction=txargs0)

signed_txn = web3.eth.account.signTransaction(rv_txn, private_key=private_key0)
...
contract_instance = web3.eth.contract(address = contract_address, abi = contract_abi, bytecode = contract_bytecode)

print("Deployment hash:", tx_hash.hex())
print("Deployment address:", contract_instance.address)
